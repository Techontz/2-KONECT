...
return [
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | Refuse an order that is too small to charge
    |--------------------------------------------------------------------------
    |
    | Off by default, and deliberately so. Stripe enforces its own minimum
    | against the *settlement* currency rather than the presented one, so the
    | true floor for a TZS order depends on the account's country and the rate
    | on the day. Guessing it here and guessing high would refuse orders Stripe
    | would happily have taken.
    |
    | Left at 0, a too-small order is refused by Stripe and the shopper is told
    | the payment could not be started. Set this — in minor units, so TZS 1,300
    | is 130000 — once you know the real floor from a declined attempt, and the
    | refusal moves here where it can be explained properly.
    |
    | A zero or negative total is refused regardless of this setting. That one
    | needs no threshold: it can never be a chargeable order.
    */
    'minimum_minor' => (int) env('STRIPE_MINIMUM_MINOR', 0),
