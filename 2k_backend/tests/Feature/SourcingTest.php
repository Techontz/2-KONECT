...
class SourcingTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        // Imports are prepaid, so the suite needs a live channel to pay with.
        \App\Models\CheckoutPaymentChannel::create([
            'code' => 'lipa_namba', 'label' => 'Lipa Namba', 'merchant_name' => '2KONECT',
            'number' => '555123', 'is_active' => true,
            'requires_reference' => true, 'requires_verification' => true, 'sort_order' => 1,
        ]);
    ...
    public function test_buying_the_imported_option_records_its_price_and_date(): void
    {
        ...
        $order = $this->postJson('/api/shop/orders', [
            ...
            'delivery_address' => 'Mikocheni',
            'customer_phone' => '0700000002',
            // The product is stocked locally but this offer imports it, so the
            // line is an import and must be prepaid. The rule reads the
            // resolved availability, not the product's own.
            'payment_method' => 'lipa_namba',
    ...
    public function test_the_timeline_speaks_in_the_right_tense(): void
    {
        ...
        // An import is prepaid, so the opening line says what is actually
        // owed rather than promising the courier will collect.
        $this->assertSame('Order received. Awaiting payment.', $steps['pending']['note']);
