<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Delete Account - 2KONECT</title>
    <style>
        body {
            font-family: Arial, sans-serif;
            max-width: 800px;
            margin: 40px auto;
            line-height: 1.6;
        }
        h1 {
            color: #222;
        }
        .box {
            background: #f9f9f9;
            border: 1px solid #ddd;
            padding: 20px;
            border-radius: 6px;
        }
    </style>
</head>
<body>
    <h1>Delete Your Account</h1>
    <p>
        If you wish to delete your 2KONECT account and associated data, please follow the instructions below.
    </p>

    <div class="box">
        <h3>Steps:</h3>
        <ol>
            <li>Email us at <a href="mailto:support@2konect.com">support@2konect.com</a></li>
            <li>Use the subject line: <strong>Delete My Account</strong></li>
            <li>Include your registered email and phone number in the message.</li>
        </ol>
        <p>
            Once we receive your request, we will process the deletion of your account and associated personal data 
            within <strong>7 business days</strong>, except where retention is required for legal or financial reasons.
        </p>
    </div>

    <p>
        For any questions, contact us at 
        <a href="mailto:support@2konect.com">support@2konect.com</a>.
    </p>
</body>
</html>
